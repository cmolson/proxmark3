...
static void print_info_result(const uint8_t *data) {
    ...
    PrintAndLogEx(INFO, "Lockbit 0: %d", (data[3] & 0x40) ? 1 : 0);
    PrintAndLogEx(INFO, "Lockbit 1: %d", (data[3] & 0x80) ? 1 : 0);
    PrintAndLogEx(INFO, "Tag is %s.", (data[3] & 0x40) ? _RED_("LOCKED") : _GREEN_("UNLOCKED"));
    PrintAndLogEx(NORMAL, "");
...
int CmdEM4x70WriteKey(const char *Cmd) {
    ...
    if (resp.status) {
        PrintAndLogEx(INFO, "Crypt key written");
        return PM3_SUCCESS;
    }

    PrintAndLogEx(FAILED, "Writing Crypt key " _RED_("Failed"));
    return PM3_ESOFT;
}
