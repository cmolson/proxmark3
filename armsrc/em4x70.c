...
#define EM4X70_T_TAG_FULL_PERIOD            (32 * TICKS_PER_FC) // 1 RF Period
...
static uint8_t gHigh = HIGH_SIGNAL_THRESHOLD;
static uint8_t gLow  = LOW_SIGNAL_THRESHOLD;
...
static int em4x70_receive(size_t length, uint8_t *bits);
...
static bool get_signalproperties(void) {
    ...
    for (int i = 0; i < EM4X70_T_TAG_FULL_PERIOD * no_periods; i++) {
        ...
        if (AT91C_BASE_SSC->SSC_RHR > gHigh) {
            return true;
        }
    }
    return false;
...
static int authenticate(const uint8_t *rnd, const uint8_t *frnd, uint8_t *response) {
    ...
    if (find_listen_window(true)) {
        ...
        int num = em4x70_receive(20, grnd);
        if (num < 20) {
...
static int send_pin(const uint32_t pin) {
    ...
    if (find_listen_window(true)) {
        ...
        if (check_ack()) {
            ...
            int num  = em4x70_receive(32, tag_id);
            if (num < 32) {
...
static bool send_command_and_read(uint8_t command, size_t resp_len_bits, uint8_t *out_bytes) {
    ...
    while (retries) {
        ...
        if (find_listen_window(true)) {
            ...
            int len = em4x70_receive(resp_len_bits, bits);
